...
use std::rc::Rc;
use crate::tools::typed_map::TypedMap;
...
impl Container {
    pub fn new() -> Self {
        Self { bindings: TypedMap::new() }
    }

    fn bind_fn<T: Any + ?Sized + 'static>(&mut self, f: Box<dyn Fn() -> Rc<T>>) {
        self.bindings.insert(f);
    }

    pub fn resolve<T: Any + ?Sized + 'static>(&self) -> Option<Rc<T>> {
        self.bindings
...
pub struct Binder<'a, T: Any + ?Sized + 'static> {
    container: &'a mut Container,
    phantom: PhantomData<T>,
}

impl Container {
    pub fn bind<T: Any + ?Sized + 'static>(&mut self) -> Binder<T> {
        Binder {
            container: self,
            phantom: PhantomData,
        }
...
impl<'a, T: ?Sized> Binder<'a, T> {
    pub fn to_singleton(self, v: Rc<T>) {
        self.container.bind_fn(Box::new(move || { return v.clone(); }));
    }
...
mod use_case {
    ...
    pub fn execute() {
        let mut container = crate::tools::di::Container::new();
