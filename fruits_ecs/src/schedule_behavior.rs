...
use crate::{data::world_data::WorldData, order_graph::OrderGraph, system::{System, SystemWithMarker}, system_data::SystemStatesHolder};
        ...
        loop {
            ...
            if let Some(system_index) = system_index {
                ...
                let job = move || {
                    ...
                    system.borrow_and_execute(&*data, &mut *system_data.try_lock().ok().unwrap());
